...
export function getAPIConfiguration() {
  const url = baseURL + ApiParameters.setApiKey(apiConstants.API_KEY);
...
export function getCountries() {
  const url =
    baseURL + "/countries" + ApiParameters.setApiKey(apiConstants.API_KEY);
  return fetch(url).then(handleResponse).catch(handleError);
}

// Get a list of the jobs and departments we use on TMDB.
export function getJobs() {
  const url = baseURL + "/jobs" + ApiParameters.setApiKey(apiConstants.API_KEY);
  return fetch(url).then(handleResponse).catch(handleError);
}
...
export function getLanguages() {
  const url =
    baseURL + "/languages" + ApiParameters.setApiKey(apiConstants.API_KEY);
  return fetch(url).then(handleResponse).catch(handleError);
}
...
export function getTimezones() {
  const url =
    baseURL + "/timezones" + ApiParameters.setApiKey(apiConstants.API_KEY);
  return fetch(url).then(handleResponse).catch(handleError);
}
